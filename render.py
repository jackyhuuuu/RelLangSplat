...
def render_set(model_path, source_path, name, iteration, views, gaussians, pipeline, background, args):
    # 定义保存渲染结果和原始图像的路径
    # render_path 和 gts_path：分别用于保存渲染结果和原始图像
    # render_npy_path 和 gts_npy_path：用于保存渲染结果和原始图像的 numpy 数组
...
def render_sets(dataset : ModelParams, iteration : int, pipeline : PipelineParams, skip_train : bool, skip_test : bool, args):
    with torch.no_grad():
        ...
        # language_feat = gaussians.get_language_feature.data
        # language_feat = language_feat.cpu().numpy()
        # np.save(os.path.join(args.model_path, "teatime_3_features" + ".npy"), language_feat)
