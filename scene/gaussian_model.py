...
class GaussianModel:
    ...
    def setup_functions(self):
        def build_covariance_from_scaling_rotation(scaling, scaling_modifier, rotation):
            ...
            # 计算实际协方差矩阵，L乘以它的专置（第一个维度是batch_size）
    ...
    def __init__(self, sh_degree : int):# sh_degree 设置最大球谐函数 (Spherical Harmonics) 的阶数
        self.active_sh_degree = 0 # 初始化为 0，表示当前活跃的球谐函数阶数
        self.max_sh_degree = sh_degree # 设置最大球谐函数 (Spherical Harmonics) 的阶数
        # 初始化为空的张量
        self._xyz = torch.empty(0) # 中心点位置即3D Gaussian的均值
        self._features_dc = torch.empty(0) # 第一个球谐系数，用来表示颜色
        ...
        self.xyz_gradient_accum = torch.empty(0) # 3D gaussian的均值的累积梯度
    ...
    def create_from_pcd(self, pcd: BasicPointCloud, spatial_lr_scale: float):
        ...
        # 将点云的坐标数据转换为 PyTorch 张量并移动到 GPU 上
        fused_point_cloud = torch.tensor(np.asarray(pcd.points)).float().cuda()
        # 将颜色数据转换为球谐函数表示 (RGB2SH)，并初始化特征张量，fused_color 的形状为 (N, 3)，表示每个点的 RGB 颜色
        fused_color = RGB2SH(torch.tensor(np.asarray(pcd.colors)).float().cuda())
        # features 张量的形状为 (N, 3, (self.max_sh_degree + 1) ** 2)
        features = torch.zeros((fused_color.shape[0], 3, (self.max_sh_degree + 1) ** 2)).float().cuda()
        # 将颜色信息赋给球谐系数为0的通道，高阶系数通道初始化为0
        features[:, :, 0] = fused_color
        features[:, :, 1:] = 0.0

        print("Number of points at initialisation : ", fused_point_cloud.shape[0])
        # 计算每个点到其最近邻点的距离，并将最小值限制为 0.0000001 以避免除零错误
        dist2 = torch.clamp_min(distCUDA2(torch.from_numpy(np.asarray(pcd.points)).float().cuda()), 0.0000001)
        # 计算这些距离的平方根，并取对数，扩展维度以匹配点的数量
        scales = torch.log(torch.sqrt(dist2))[..., None].repeat(1, 3)
        # rots 初始化为单位四元数 (w, x, y, z)，其中 w 分量为 1，表示没有旋转
        rots = torch.zeros((fused_point_cloud.shape[0], 4), device="cuda")
        rots[:, 0] = 1
        # 初始化不透明度，并将其值通过反 sigmoid 函数映射
        opacities = inverse_sigmoid(0.1 * torch.ones((fused_point_cloud.shape[0], 1), dtype=torch.float, device="cuda"))
        # language_feature = torch.zeros((fused_point_cloud.shape[0], 512), device="cuda")

        # 将点云坐标、特征、不透明度、尺度和旋转初始化为 PyTorch 参数，这些参数将在训练过程中被优化
        # contiguous() 用于保证张量在内存中的布局是连续的
    ...
    def _prune_optimizer(self, mask):
        optimizable_tensors = {}
        for group in self.optimizer.param_groups:
            # 首先检查优化器状态中是否存储了参数的状态信息
            stored_state = self.optimizer.state.get(group['params'][0], None)
            # 如果存储了参数的状态信息，则根据掩码（mask）修剪这些状态信息，并从优化器状态中删除原始参数并替换为修剪后的参数
            if stored_state is not None:
                stored_state["exp_avg"] = stored_state["exp_avg"][mask]
                stored_state["exp_avg_sq"] = stored_state["exp_avg_sq"][mask]
                # 删除优化器状态中原始参数对应的状态信息
                del self.optimizer.state[group['params'][0]]
                # 并将参数组中的参数替换为根据掩码修剪后的参数, 同时确保新参数的梯度需要计算（requires_grad=True）
                group["params"][0] = nn.Parameter((group["params"][0][mask].requires_grad_(True)))
                self.optimizer.state[group['params'][0]] = stored_state
                # 将修剪后的参数状态信息重新添加到优化器状态中
                optimizable_tensors[group["name"]] = group["params"][0]
            # 如果没有存储参数的状态信息，则直接将参数替换为修剪后的参数
            else:
                group["params"][0] = nn.Parameter(group["params"][0][mask].requires_grad_(True))
                optimizable_tensors[group["name"]] = group["params"][0]
        return optimizable_tensors
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
        ...
        # 根据梯度阈值（grad_threshold）筛选点
        selected_pts_mask = torch.where(padded_grad >= grad_threshold, True, False)
        # 根据场景范围（scene_extent）筛选点，点的缩放比例要大于指定的百分比（self.percent_dense）乘以场景范围
        selected_pts_mask = torch.logical_and(selected_pts_mask,
                                              torch.max(self.get_scaling, dim=1).values > self.percent_dense*scene_extent)

        # 对于满足条件的点，生成N倍（2倍）的样本，并且以这些点为中心生成一个新的局部坐标系
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent):
        ...
        # 根据场景范围（scene_extent）筛选点，点的缩放比例要小于指定的百分比（self.percent_dense）乘以场景范围
        selected_pts_mask = torch.logical_and(selected_pts_mask,
                                              torch.max(self.get_scaling, dim=1).values <= self.percent_dense*scene_extent)
    ...
    def densify_and_prune(self, max_grad, min_opacity, extent, max_screen_size):
        ...
        # 删除不透明度小于阈值的3D Gaussian
        prune_mask = (self.get_opacity < min_opacity).squeeze()
        if max_screen_size:
            # 删除2D半径超过2D尺寸阈值的3D Gaussian
            big_points_vs = self.max_radii2D > max_screen_size
            # 计算在每个维度上的最大缩放值，然后检查是否大于0.1乘以给定的范围（extent）
            big_points_ws = self.get_scaling.max(dim=1).values > 0.1 * extent
            # 求所有不符合条件的Gaussian的或集合
